...
@Data
public class ProductCollectionReq extends BaseProductCollectReq {

    @ApiModelProperty(value = "产品图片Base64", example = "xxx",hidden = true)
    private String productImageBase64;

    @ApiModelProperty(value = "文件信息", hidden = true)
    private List<AttachmentDataDTO> documentsFiles;

    public ProductCollectionReq validData(IRemoterApi remoterApi) {
...
//            AssertUtil.isTrue(StringUtils.isNotBlank(super.getBindCodeName()), "附带包材选项,需要选择附带包材");
    ...
    public ProductCollectionReq calculateTheVolume() {
     ...
     * @return
     */
    public ProductCollectionReq checkPack(BasePackingClientService basePackingFeignService) {
        // 3、选择物物流包装OMS要校验是否存在；
    ...
    public ProductCollectionReq setTheCode(IRemoterApi iRemoterApi, DocSubConfigData docSubConfigData) {
    ...
    public ProductCollectionReq uploadFile(IRemoterApi remoterApi) {
